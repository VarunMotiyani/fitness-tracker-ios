...
@Suite struct MemoryKeeperCoordinatorTests {
    ...
    // Chat-exchange memory extraction moved into AskCoachCoordinator's own
    // reply call (see AskCoachCoordinatorTests) — MemoryKeeperCoordinator now
    // only ever runs on a finished session.
}
