...
enum BackupRestoreService {
    ...
    static func exportData(context: ModelContext, defaults: UserDefaults = .standard) throws -> Data {
        ...
        let messages = rows(ChatMessageModel.self).map { ["id": $0.id.uuidString, "conversationID": $0.conversationID, "role": $0.role, "text": $0.text, "timestamp": date($0.timestamp)] as [String: Any] }
        let summaries = rows(ChatSummaryModel.self).map { ["conversationID": $0.conversationID, "text": $0.text, "updatedAt": date($0.updatedAt), "messagesCoveredThrough": optional($0.messagesCoveredThrough.map(date))] as [String: Any] }
    ...
    static func restore(data: Data, into context: ModelContext, defaults: UserDefaults = .standard) throws {
        ...
        for m in rows("chatMessages") { guard let role = str(m["role"]), let text = str(m["text"]), let time = date(m["timestamp"]) else { continue }; let x = ChatMessageModel(role: role, text: text, timestamp: time, conversationID: str(m["conversationID"]) ?? "default"); if let restoredID = id(m["id"]) { x.id = restoredID }; context.insert(x) }
        for s in rows("chatSummaries") { guard let text = str(s["text"]), let updated = date(s["updatedAt"]) else { continue }; let x = ChatSummaryModel(conversationID: str(s["conversationID"]) ?? "default"); x.text = text; x.updatedAt = updated; x.messagesCoveredThrough = date(s["messagesCoveredThrough"]); context.insert(x) }
    ...
    /// Everything that counts as "history/state" rather than account setup —
    /// shared by ``reset(context:defaults:)`` (full factory reset) and
    /// ``clearHistoryKeepingProfile(context:)`` (the coach's `clear_data`
    /// tool), so there's one list to keep in sync, not two.
    private static let historyModelTypes: [any PersistentModel.Type] = [
        StoredPlan.self, CustomExerciseModel.self, AICallRecord.self,
    ...
    static func reset(context: ModelContext, defaults: UserDefaults = .standard) throws {
        ...
        let types: [any PersistentModel.Type] = historyModelTypes + [UserProfile.self, ProviderProfile.self]
    ...
    /// The coach's `clear_data` tool: wipes workout/plan/chat/memory history
    /// but leaves the athlete's `UserProfile` and `ProviderProfile` (AI
    /// provider setup, API keys) untouched — "all data other than profile,"
    /// not a full factory reset. No UserDefaults/Keychain cleanup either,
    /// since none of that is athlete history.
    static func clearHistoryKeepingProfile(context: ModelContext) throws {
        if let custom = try? context.fetch(FetchDescriptor<CustomExerciseModel>()) { custom.forEach { CustomExercisePhotoStore.delete(filename: $0.photoFilename) } }
        for type in historyModelTypes { try context.delete(model: type) }
        try context.save()
    }
}
