# UI Parity Gap Doc — PulseAI vs. reference app

A screen-by-screen catalogue of what the reference app web app does that the Swift app does not
yet, why it matters, where the behaviour is defined in the reference app reference checkout
(`~/Documents/person/reference-app`), and how to build it natively in SwiftUI.

**How to read the "reference app ref" column:** it is a *pointer* into the reference codebase so
...
port of reference app's source.
...
| **reference app ref** | `sheets.jsx` → `exConfigSheet` and its `ProgressionFields` sub-component (policy picker + increment + rep-range inputs); `lib/history.js` → `defaultConfig`; `lib/rep-range.js` → `normalizeRepRange` (already ported as `RepRangeNormalize`). |
...
| **reference app ref** | `views/Plan.jsx` — the "Week schedule" list (`weekOrder(weekStartOf(S)).map` → `dayAssignSheet`), the "Routines" section with the "New" button and `addRoutine`, the empty state, and the header `planToolsSheet` button. `sheets.jsx` → `dayAssignSheet`. |
...
| **Status** | Missing. |
| **reference app ref** | `sheets.jsx` → `planToolsSheet`, `planImportSheet`; `lib/plan-share.js` (encode/merge). |
...
| **reference app ref** | `sheets.jsx` → `glyphPicker`; `lib/glyphs.js` (the glyph set + `glyphOf`). |
...
| **reference app ref** | `views/Workout.jsx` — the `ExerciseBlock` set-row grid, the `col2`/effort column and `stepEffort` from `lib/history.js` (`EFFORT`, `stepEffort`, `capEffort`). |
...
| **reference app ref** | `views/Workout.jsx` — the "Make superset with next" button and `onPairNext`; `lib/supersetFlow.js`. |
...
| **reference app ref** | `views/Workout.jsx` — the header progress bar + "Exercise {i} / {n}" and the bottom `Prev`/`Next` buttons around the rest-timer control. |
...
| **reference app ref** | `views/Workout.jsx` — "Add warm-up set" / "Remove set"; `lib/history.js` — `insertWarmupRow`, `rerampWarmups`, `MAX_PLANNED_WARMUPS`. |
...
| **Status** | Missing. |
| **reference app ref** | `views/Workout.jsx` — the "Last time (...)" line; built from `lastEntryFor` in `lib/history.js`. |
...
| **reference app ref** | `views/Workout.jsx` — the yellow "why" line above the set table; `nextPrescription().why` in `lib/progression.js`. |
...
| **Status** | Missing. |
| **reference app ref** | `sheets.jsx` → `topWeightSheet` / `TopWeight`; the `exWeights` map in `lib/history.js` / `useStore.js`. |
...
| **reference app ref** | `sheets.jsx` → `workoutCompleteSheet` / `WorkoutComplete`. |
...
| **reference app ref** | `views/Workout.jsx` — the `toggle` handler and its `setProgressHighWater` / `restAfterSet` / `restSecFor` / `supersetFlowStep` calls; `lib/supersetFlow.js` JSDoc for the uneven-round and re-check rules. |
...
| **reference app ref** | `sheets.jsx` → `swapActiveWorkoutExercise`; `lib/active-exercise-swap.js`. |
...
| **reference app ref** | `views/Workout.jsx` — the `exnote` lines + the header pencil button; `sheets.jsx` → `exerciseNoteSheet`, `sessionNoteSheet`; `lib/history.js` → `pinnedNoteFor`, `NOTE_MAX`; `lib/finish-workout.js` — how `note`/`notePin` are written. |
...
| **reference app ref** | `views/Workout.jsx` — the `.subrow` blocks; `lib/workout-model.js` — `addDrop`/`addCluster`, `nextDropWeight`, `nextBurstReps`, `splitBurstReps`, `extraVolumeOf`. |
...
| **reference app ref** | `views/Workout.jsx` — the media card + "Expand"; `components/Media.jsx`. |
...
| **Status** | Home-only. |
| **reference app ref** | `views/Stats.jsx` — the "Body weight" `card` with the `Segmented` window and `LineChart points={bwPts} goal={S.targetW}`. |
...
| **reference app ref** | `views/Stats.jsx` — the "Exercise progress" `card`: the `SelectRow` exercise picker with search, the `Segmented` [Top set / Est. 1RM / Effort], and the caption lines including `e1Best`. |
...
| **reference app ref** | `views/Stats.jsx` — the "Where the sets land" block in `EffortCard`; `lib/effort.js` → `effortHistogram`. |
...
| **reference app ref** | `views/Stats.jsx` — `MuscleCard`: the `Segmented` [balance/fatigue/strength], the three `BodyMap` variants with `FATIGUE_LEVELS` / `STRENGTH_LEVELS`, `FatigueLegend` / `StrengthLegend`, and the per-muscle exercise list. |
...
| **Status** | Missing. |
| **reference app ref** | `views/Stats.jsx` — the "Not trained in this period" / "No hard sets in this period" section and the `onMuscle`/`sel` tap handling. |
...
| **reference app ref** | `views/Stats.jsx` — the "Recent workouts" section + `WorkoutRow` + the "All {n}" button; `sheets.jsx` → `workoutDetailSheet`. |
...
| **reference app ref** | `views/Stats.jsx` — the `<Heatmap onDay={...}>` handler. |
...
| Row | What | reference app ref | How to add |
|---|---|---|---|
...
| Item | What | reference app ref | How to add |
|---|---|---|---|
...
| **reference app ref** | `lib/backfill.js`; `lib/session-start.js` (`buildSessionEntries` — one path for live + backfill); `views/Workout.jsx` — the `A.backfill` branch (banner, no `startRest`). |
...
The persisted model needs these fields (keep names reference app-JSON-compatible where you can so
an reference app export imports):
