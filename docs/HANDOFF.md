...
_Living document. Last updated: 2026-08-29 (Phase 2a built + reviewed on its branch; awaiting Varun's merge decision)._
...
| | |
|---|---|
| **Phase** | **Phase 1 (1a/1b/1c) MERGED to `main`.** App today: onboarding → **AI-or-rule** weekly plan → read-only plan view; `ProviderProfile` + Keychain keys, 3 `LLMProvider` adapters, `PlanCoordinator` (AI-generate → validate → retry-once → rule fallback), `AICallRecord` cost ledger + `$` chip / Usage. `FitnessCore` = 5 modules. Tests: app 32 Swift-Testing + 6 XCTest, `FitnessCore` 35 — all green on `main`. **Phase 2a BUILT (branch, not merged)** — see Branch + Next action rows. It adds NO app screens; it is pure `FitnessCore` engine work (`Metrics` + `CoachMemory` modules + real `ProgressionRule`/`FinalizeGuardrail`). User-visible pages arrive in **2b** (session runner), **2c** (AI coach wiring), **2d** (history + coach-memory screen + pick-a-split). |
| **Next action** | **Review + merge Phase 2a**, then Phase 2b. Phase 2 design spec: `docs/superpowers/specs/2026-08-29-phase-2-session-runner-design.md` (session runner + comprehensive metrics + self-curating coach memory + agent layer; chat interface split out as "Phase 2-chat"). Split into sub-plans 2a/2b/2c/2d. **2a is BUILT on branch `phase-2a-metrics-memory-foundation`** (see Branch row) — pure `FitnessCore` logic, not pushed/merged, waiting for you. Carry forward: **`docs/superpowers/plans/2026-08-29-phase-2a-followups.md`** (committed copy of every deferred/parked Minor from the 2a run — R1 dangling `supersededBy` for 2b, R2 light-load clamp for 2c, the T5 window-vs-bucket note for 2d, `MemorySource` wire format, etc.), the `FitnessCore/README.md` follow-ups, and the 1c deferrals (budget cap, native Anthropic/Vertex/Bedrock, vision). |
...
| **Branch** | `main` at `bcb3189` (1c merged + Phase 2 spec). **Active work branch: `phase-2a-metrics-memory-foundation` @ `2f91336`** — 21 commits (12 SDD tasks + 9-commit fix wave), **126 `FitnessCore` tests green**, `FitnessCore`-package-only (no app changes). Built subagent-driven; whole-branch Opus review returned "merge with fixes" → all 1 Critical + 8 Important fixed + scoped re-review clean; 5 Minors parked (ledger R1–R5). **NOT pushed, NOT merged** — an autonomous session built it while Varun slept, under a "don't push/merge anything" instruction. Varun: review the branch, then merge to `main` (or PR) yourself. 2b branches off this once 2a lands. SDD workspace kept at `.superpowers/sdd/2026-08-29-phase-2a-metrics-memory-foundation/` (ledger + all reviews). |
