...
| **Next action** | **Review + merge Phase 2a**, then Phase 2b. Phase 2 design spec: `docs/superpowers/specs/2026-08-29-phase-2-session-runner-design.md` (session runner + comprehensive metrics + self-curating coach memory + agent layer; chat interface split out as "Phase 2-chat"). Split into sub-plans 2a/2b/2c/2d. **2a is BUILT on branch `phase-2a-metrics-memory-foundation`** (see Branch row) — pure `FitnessCore` logic, not pushed/merged, waiting for you. Carry forward: the parked Minors in the 2a SDD ledger (R1–R5), the `FitnessCore/README.md` follow-ups, and the 1c deferrals (budget cap, native Anthropic/Vertex/Bedrock, vision). |
